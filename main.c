...
int main()
{
	...
	/* Preenche lista para comparacao no final do dia */
	ListaComparacao=InitializeWarehouse(ListaComparacao);
	/* Limpa erros de memória */
	CleanMemoryErrors(ListaProdutos, ListaProdutos);
	CleanMemoryErrors(ListaComparacao, ListaComparacao);
    DisplayMenu();
    do
        {
            DisplayMode=GetDisplayMode();
            ...
            switch(answer_menu)
                                ...
                                case corredor:
                                    ...
                                    if(DisplayMode==1)
                                    ...
                                    else
                                        {
                                            ...
                                            printf("Introduza o nome do ficheiro sem extensao: ");
                                            scanf("%s", &filename);
                                            GuardaPesquisabyCorredor(ListaProdutos, filename, search_corredor);
                                            LINE_BREAK();
                                            DisplayMenuVisualizacoes();
                                        }
                                    break;
                ...
                case gerir:
					...
					do
						{
							printf("\nInsere uma opcao valida:\n");
							scanf("%d", &answer_encomendas);
							switch(answer_encomendas)
								{
								case processar:
									printf("Introduza o nome do ficheiro sem extensao: ");
                                    scanf("%s", &filename);
									ProcessaEncomenda(filename, ListaProdutos, ListaProdutos);
									DisplayMenuEncomendas();
									break;
								}
						}
					while(answer_encomendas != sair_encomendas);
					DisplayMenu();
                    break;
                case stocks:
                    CLEAR_SCREEN();
	...
	ReportSales(ListaProdutos, ListaComparacao);
    ActualizaBIN(ListaProdutos);
